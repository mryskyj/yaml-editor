...
struct識別子は `<パッケージ名>.<構造体名>` とする。

Schema registryはroot schemaとは別に、参照用サンプルスキーマのstruct一覧を保持できるようにする。
参照用サンプルスキーマはGoソースを静的解析し、YAML対象フィールドを持つstructを `<パッケージ名>.<構造体名>` で引ける形にする。

補完時の責務:

- `tool` の値位置では、参照用サンプルスキーマの `<パッケージ名>.<構造体名>` を候補に出す
...
- `dayN.holiday` に `true` または `false` が入力された行で改行した場合、次の空行に `dayN+1`, `date`, `holiday` のブロックを挿入する
- 直前の `dayN.date` が `YYYY-MM-DD` 形式の場合、次の `date` にはUTC基準で翌日の日付文字列を入れる
- 既に次の `dayN` が存在する場合は重複挿入しない
