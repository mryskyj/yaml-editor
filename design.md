...
- Windows GUI exeではコンソールが表示されないため、起動時オプションや外部スキーマ読み込みの失敗はプロセス終了ではなくUIの起動診断として表示する
...
App serviceの `StartupDiagnostics` は、起動時オプションや外部スキーマ読み込みの非致命的な失敗をUIへ返す。
...
起動時オプションはWails起動前に専用のflag setで解析し、解析後はWailsのWindowsファイル関連付け処理に干渉しないようプロセス引数を実行ファイル名だけに戻す。
オプション解析または外部スキーマ読み込みに失敗した場合は、組み込みRootスキーマで起動し、Frontendが `StartupDiagnostics` APIから取得した診断を画面上部に表示する。
