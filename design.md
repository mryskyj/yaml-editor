...
App serviceの `LoadExternalSchema` は、起動後に選択されたGoソースフォルダを解析し、root schemaと参照用tool schemaを登録済みスキーマとして差し替える。
...
- 選択フォルダを `App.LoadExternalSchema` へ渡し、成功時にSchema pane、Root schema、初期YAMLテンプレートを再読み込みする
- Noまたはキャンセルの場合は組み込みRootスキーマと組み込み参照用サンプルスキーマを使う
- 読み込み失敗時はエラー内容をダイアログ表示し、組み込みスキーマのまま継続する

外部スキーマ未選択時は組み込みの `app/rootschema` ソースを読み込み、`File` をroot structとして登録する。
外部スキーマを指定する場合は特定のGo型名に固定せず、root候補の自動検出を使う。
...
root候補が0件または複数ある場合は曖昧なためエラーとし、既存スキーマを維持する。
...
外部スキーマを選択した場合も、root候補が1つであればroot struct名の明示指定は不要とする。
