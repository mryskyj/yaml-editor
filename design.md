...
アプリ起動時に指定フォルダ内の外部Goソースファイルから対象structを読み込み、解析済みスキーマを保持する。
...
起動時の外部Goソース読み込みでは、Goコードをコンパイル・実行せず、指定フォルダ直下の`.go`ファイル内のstruct定義とタグだけを静的解析する。
...
`--schema-dir` 未指定時は組み込みの `app/sampleschema.Config` を読み込む。
...
1. Goバックエンドで外部Goソースフォルダと対象struct名を決定する
2. Struct parserがフォルダ内のGoソースを静的解析して内部スキーマを生成する
