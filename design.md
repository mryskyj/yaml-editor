...
- Windows GUI exeではコンソールが表示されないため、外部スキーマ読み込みは起動後のGUI操作で行い、失敗時はプロセス終了ではなくUIダイアログとして表示する
...
App serviceの `LoadExternalSchema` は、起動後に選択されたGoソースフォルダを解析し、参照用tool schemaだけを登録済みスキーマとして差し替える。
YAML文書全体を表すroot schemaは組み込みの `app/rootschema.File` を維持し、外部スキーマ読み込み後も `common`、`scenario`、`steps` の補完・検証を継続する。
外部スキーマ読み込みに失敗した場合はエラーを返し、既存の登録済みスキーマは変更しない。
...
外部スキーマを読み込まない場合は `app/sampleschema`、外部スキーマフォルダを選択した場合は指定フォルダから登録したtool schemaを表示対象にする。
...
外部Goソースフォルダが起動後に選択された場合は、指定フォルダ内のGoソースファイルから対象structを静的解析して登録する。
...
外部Goソース読み込みでは、Goコードをコンパイル・実行せず、指定フォルダ直下の`.go`ファイル内のstruct定義とタグだけを`go/parser`で静的解析する。

`*_test.go` は読み込み対象外とする。

起動後の外部スキーマ設定:

- フロントエンドは初期表示時に「外部スキーマを読み込みますか？」を確認する
- 確認にはブラウザ標準の `window.confirm` ではなくWailsのネイティブQuestionダイアログを使い、macOSとWindowsで同じ起動時挙動にする
- Yesの場合はWailsのOSフォルダ選択ダイアログでGoソースフォルダを選ぶ
- 選択フォルダを `App.LoadExternalSchema` へ渡し、成功時にSchema paneと初期YAMLテンプレートを再読み込みする
- Noまたはキャンセルの場合は組み込みRootスキーマと組み込み参照用サンプルスキーマを使う
- 読み込み失敗時はエラー内容をダイアログ表示し、組み込みスキーマのまま継続する

組み込みの `app/rootschema` ソースを読み込み、`File` をroot structとして登録する。
外部スキーマを選択した場合もroot structは差し替えず、選択フォルダは参照用tool schemaとして読み込む。
root schemaはYAML文書全体の検証・補完に必要な内部概念として保持する。

外部Goソースフォルダ解析では、読み込み対象ファイル内のトップレベル `type Xxx struct` を参照用tool schemaとして解析する。
フィールド型が同一パッケージ内で定義された名前付きstructを参照している場合は、そのstruct定義へ解決して内部スキーマに展開する。
フィールド型が `pkg.Type` 形式でimport先パッケージを参照している場合は、Goファイルのimport宣言からimport pathを取得し、GOPATH配下の該当パッケージディレクトリを静的解析して内部スキーマに展開する。
...
- サブディレクトリ配下のGoファイル
- GOPATH配下から解決できないimport先パッケージの型
- 標準ライブラリパッケージの型
- type aliasのみで定義された型
- generic型
- 循環参照するstruct
...
外部スキーマを読み込まない場合は `app/sampleschema`、外部スキーマフォルダを選択した場合は指定フォルダ内のGo structを参照用サンプルスキーマとして登録する。
...
`scenario.steps[].day_ref` の値補完では、Completion providerが現在のYAML本文を解析し、`common.dates` 配下に定義済みのdayキーを候補として返す。
候補のdescriptionには `date` と `holiday` の値を入れ、Monaco Editorの補完詳細で参照できるようにする。
フロントエンドでは `day_ref` 候補だけ通常の型情報表示を使わず、候補ラベルの横に `date` と `holiday` を表示する。
候補は固定値ではなく、編集中のYAML本文に存在する `common.dates` の内容から毎回生成する。
...
外部スキーマを選択した場合もroot schemaは差し替えず、選択フォルダのGo structは参照用tool schemaとして登録する。
