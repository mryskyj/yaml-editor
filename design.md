...
struct識別子は `"<パッケージ名>.<構造体名>"` とし、YAML上ではダブルクォートで囲む。
...
- `tool` の値位置では、参照用サンプルスキーマのstruct識別子を `.` の前後で分割し、`パッケージ名.` と `構造体名` を段階的に候補に出し、確定時はダブルクォート付きの値を挿入する
...
テンプレートには `id`, `name`, `day_ref`, `schedule_ref`, `action.tool`, `action.args` を含める。
