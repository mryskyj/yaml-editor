...
アプリ起動時に外部Goソースファイルから対象structを読み込み、解析済みスキーマを保持する。

MVPでは複数スキーマ切り替えを行わないため、アプリ内で利用するルートスキーマは1つとする。

主な責務:

- 外部Goソースファイルからstructを登録
- 解析済みスキーマの保持
- ルートスキーマの取得
...
起動時の外部Goソース読み込みでは、Goコードをコンパイル・実行せず、`.go`ファイル内のstruct定義とタグだけを静的解析する。

起動時スキーマ設定:

- `YAML_STRUCT_SCHEMA_FILE`: 外部Goソースファイルのパス
- `YAML_STRUCT_SCHEMA_TYPE`: ルートstruct名

未指定時は `schemas/sample_schema.go` の `Config` を読み込む。
...
1. Goバックエンドで外部Goソースファイルと対象struct名を決定する
2. Struct parserがGoソースを静的解析して内部スキーマを生成する
