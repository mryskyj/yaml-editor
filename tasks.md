...
- `Cmd/Ctrl + N` で新規タブを作成する
- `Cmd/Ctrl + O` でYAMLファイルを開く
- `Cmd/Ctrl + S` でアクティブなタブを保存する
- `Cmd/Ctrl + W` でアクティブなタブを閉じる
- `Cmd/Ctrl + Tab` で次のタブへ切り替える
- `Cmd/Ctrl + Shift + Tab` で前のタブへ切り替える
- `Esc` でアプリ内確認ダイアログを閉じる
