...
- [x] ファイルタブUIを実装する
- [x] タブ単位の保存・検証・補完連携を実装する
- [x] 最後のタブを閉じてタブ0件状態にできるようにする
- [x] ショートカットキー要件を整理する
- [x] ショートカットキーを実装する
...
- [x] `tool` の選択値に応じた `args` 補完・検証を実装する
- [x] `args` 補完でslice要素のYAMLリスト展開を実装する
...
- [x] 起動後の外部スキーマ選択式読み込みを実装する
- [x] 外部スキーマ読み込み時も組み込みRootスキーマを維持する
- [x] 外部スキーマ読み込み確認をWailsネイティブダイアログへ変更する
- [x] 外部スキーマのimport先パッケージ型をGOPATHから解決する
...
- import先パッケージの `pkg.Type` 形式参照をGOPATH配下から解決し、参照先のstructと名前付きスカラー型を内部スキーマへ展開するようにした
...
- slice / array のリスト要素配下で要素structのキーを補完候補として返すようにした
...
- 新規作成APIはRootスキーマのRequiredキーだけを初期YAMLとして返すようにした
- Rootスキーマ初期値を `app/rootschema/defaults.yaml` から読み込むようにした
- scheduleデフォルトテンプレート取得APIを追加した
- YAML検証APIを追加した
- YAML補完APIを追加した
- 登録済みスキーマ取得APIを追加した
...
### エディタのコピー・切り取り・貼り付け操作を実装する

- ToolbarにCopy、Cut、Pasteを追加した
- エディタ領域の右クリックメニューにCopy、Cut、Pasteを追加した
- 選択範囲がある場合は選択範囲をコピー・切り取り対象にした
- 選択範囲がない場合は現在行をコピー・切り取り対象にした
- Pasteは選択範囲を置き換え、選択範囲がない場合はカーソル位置へ挿入するようにした
- PasteとCutによる編集はMonacoのUndo履歴に1操作として追加するようにした

### `Shift + 矢印キー` の範囲選択を実装する
...
- 参照用tool schemaのサブディレクトリを再帰的に読み込み、相対ディレクトリを `.` 区切りの名前空間として扱うようにした
- `tool` の値位置で `cloud.`、`ecs.`、`RunTask` のように階層付き名前空間を段階補完できるようにした
- 名前付きスカラー型、const、メソッド、YAML対象外struct、slice、mapが混在するsampleschemaを追加し、YAML対象structだけを解析できるようにした
