...
- `day_ref` の動的day候補補完
- `schedule_ref` の動的schedule候補補完
- `tool` 値に応じた `args` の補完・検証
- 起動後のフォルダ選択による外部Goソーススキーマ読み込み
...
外部Goソースをスキーマとして使う場合は、アプリ起動後の確認ダイアログでYesを選び、Goファイルを含むディレクトリを選択します。
YAML文書全体を表すroot schemaは常に組み込みの `app/rootschema.File` を使い、選択フォルダは `tool` / `args` 用の参照スキーマとして読み込みます。

Noまたはキャンセルを選んだ場合は、組み込みRootスキーマと組み込み参照用サンプルスキーマで起動します。
外部スキーマ読み込みに失敗した場合はエラー内容を画面上のダイアログで表示し、組み込みスキーマのまま継続します。

参照スキーマは選択フォルダから再帰的に読み込み、サブディレクトリを `.` 区切りの名前空間として扱います。
