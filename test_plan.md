...
- `vendor/modules.txt` が存在する場合、`scripts\build-windows-offline.ps1` はGo依存を取得せず `-mod=vendor` でWindows exeを作成できる
- `vendor/modules.txt` が存在しない場合、`scripts\build-windows-offline.ps1` は明示エラーを出す
...
Go依存もオフライン端末へ持ち込む場合は、インターネット接続可能な開発端末で以下を実行し、生成された `vendor` を含める。
