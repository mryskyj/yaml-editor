...
- 新規ファイル状態の初期YAMLには `schema_version: "1.0.0"`、`common.dates.day1`、デフォルトの `common.schedules.run1` から `run3` を含める
- 新規ファイル状態の初期YAMLでは `day_ref` と `schedule_ref` を引用符付き空文字ではなく空値で出力できる
