...
- 配布用exeを `--schema-dir` / `--schema-type` 付きで起動しても、Wails側のファイル関連付け引数処理に干渉されず通常起動する
- 配布用exeはコンソールを表示しないため、起動時オプションや外部スキーマ読み込みに失敗した場合も即終了せず、組み込みRootスキーマで起動して画面上部に診断を表示する
...
外部スキーマ読み込みに失敗した場合、GUIアプリは即終了せず、組み込みRootスキーマで起動して起動診断を表示する。
