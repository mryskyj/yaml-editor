...
- 外部Goソーススキーマは起動後の確認ダイアログとフォルダ選択で読み込む
- 配布用exeはコンソールを表示しないため、外部スキーマ読み込みに失敗した場合も即終了せず、エラー内容を画面上のダイアログで表示し、組み込みRootスキーマで継続する
...
- YAMLファイルを開く
- アプリ内のOpen操作ではOSのファイル選択ダイアログから実ファイルパスを取得し、バックエンド経由で開く
...
- 初期YAMLのRoot `schema_version` は `1.0.0` とし、`common.dates` は `day1.date` と `day1.holiday`、`common.number_of_days` は `1`、`common.schedules` は定義ファイルのデフォルトscheduleを含める
...
- `common` は同一YAML内のインライン定義、または `common: !include "relative/path.yaml"` による外部ファイル参照で定義できる
- 起動時と新規作成時の初期YAMLでは、未保存状態でも単体で検証できるようにインライン `common` を補填する
- タブごとに編集中の内容、ファイルパス、保存状態、検証結果を保持する
- 保存先が未確定、または未保存変更があるタブを閉じる場合は確認する
- 最後のタブも閉じることができ、タブが0件の場合は空の編集状態を表示する
- 最近開いたファイルを保持する
...
組み込みサンプルスキーマに加えて、起動後に選択したフォルダ内の外部Goソースファイルから対象structを読み込み、登録できるようにする。

外部Goソースファイルはフォルダ選択時に1回だけ静的解析する。
Goコードのコンパイル、実行、動的更新は行わない。

起動後の指定:

- アプリ起動後に「外部スキーマを読み込みますか？」を確認する
- Yesの場合はOSのフォルダ選択ダイアログでGoソースファイルを含むフォルダを選ぶ
- Noまたはキャンセルの場合は組み込みRootスキーマと組み込み参照用サンプルスキーマを使う

外部スキーマフォルダを選択した場合、指定フォルダ直下の複数 `.go` ファイルを読み込む。
...
YAMLタグ付きフィールドを持ち、他structから参照されていないstructが1つだけの場合はrootとして自動検出する。
root候補が0件または複数ある場合、外部スキーマ読み込みは失敗として扱い、GUIアプリは即終了せず、エラー内容を画面上のダイアログで表示して組み込みRootスキーマで継続する。
...
外部スキーマを読み込まない場合は組み込みの `app/sampleschema`、外部スキーマフォルダを選択した場合は指定フォルダ内のGo structを参照用スキーマとして扱う。
...
`common.number_of_days` は `dates` と `schedules` の間に置くint64の必須項目とする。
`common` は以下のように同一YAML内へインライン定義できる。
...
または、以下のように外部ファイル参照できる。

```yaml
common: !include "../common/common.yaml"
```

include先は、`common` の中身だけを書く形式と、`common:` をトップレベルに持つ形式の両方に対応する。
...
`!include` は `common` キーのみ対応する。
includeパスは親YAMLファイルの保存場所からの相対パスとして解決する。
未保存ファイル、または実ファイルパスを保持していないタブではincludeパスを解決できないため診断を出す。
絶対パスは当面禁止する。
include先にさらに `!include` がある場合は診断を出す。
親ファイル保存時にinclude先ファイルは保存しない。
インライン定義とinclude定義は同じ意味として検証・補完し、`day_ref` と `schedule_ref` の候補生成にも使う。
...
- アクティブなタブのエラー一覧に表示
- エラークリックで該当位置へジャンプ
- エラー一覧の行番号とメッセージは範囲選択でき、OS標準のコピー操作でコピーできる
...
外部スキーマを読み込まない場合は組み込みの `app/sampleschema` を表示対象とし、外部スキーマフォルダを選択した場合は指定フォルダから登録した参照用スキーマを表示対象とする。
...
`common` の `number_of_days` は、`dates` と `schedules` の間に置く必須int64値とする。
